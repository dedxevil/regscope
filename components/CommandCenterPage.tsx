...
const CommandCenterPage: React.FC<CommandCenterPageProps> = ({ onLogout, theme, onToggleTheme }) => {
    ...
    const [autoRefreshInterval, setAutoRefreshInterval] = useState(0); // in ms, 0 is off
    const [refreshCountdown, setRefreshCountdown] = useState<number | null>(null);

    const handleRefreshCharts = useCallback(() => {
        setIsDashboardLoading(true);
        setTimeout(() => {
            setIngredientsData(generateRandomizedIngredients());
    ...
    }, [autoRefreshInterval, handleRefreshCharts]);

    // Effect for handling the countdown timer
    useEffect(() => {
        let countdownInterval: number | undefined;
        if (autoRefreshInterval > 0) {
            setRefreshCountdown(autoRefreshInterval / 1000); // Start countdown immediately
            countdownInterval = window.setInterval(() => {
                setRefreshCountdown(prev => {
                    if (prev === null || prev <= 1) {
                        return autoRefreshInterval / 1000; // Reset
                    }
                    return prev - 1;
                });
            }, 1000);
        } else {
            setRefreshCountdown(null); // Turn off countdown
        }
    ...
    const IngredientListPanel = ({ isCollapsed }: { isCollapsed: boolean }) => (
      <>
        <div className={`p-4 border-b dark:border-gray-700 flex items-center justify-between flex-shrink-0 ${isMobileMenuOpen && 'bg-white dark:bg-spotify-card'}`}>
            {!isCollapsed && <h2 className="text-lg font-bold">Ingredients</h2>}
            <button onClick={() => isMobileMenuOpen ? setIsMobileMenuOpen(false) : setIsIngredientListCollapsed(!isCollapsed)} className="p-1 text-gray-500 hover:text-gray-800 dark:text-spotify-gray dark:hover:text-white" title={isCollapsed ? 'Expand' : 'Collapse'}>
                {isMobileMenuOpen ? <CloseIcon className="w-6 h-6"/> : (isCollapsed ? <ExpandRightIcon className="w-6 h-6" /> : <CollapseLeftIcon className="w-6 h-6" />)}
            </button>
        </div>
    ...
    return (
        <div className="h-full flex flex-col">
            {/* FIX: Pass auto-refresh state and callbacks to Header to enable feature and fix prop error */}
            <Header 
            ...
            <main className="flex-grow p-2 md:p-4 flex gap-2 md:gap-4 min-h-0 relative">
                ...
                {/* Mobile Menu */}
                <div className={`fixed md:hidden top-0 left-0 h-full w-4/5 max-w-sm bg-white dark:bg-spotify-card shadow-lg z-30 transform transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                    <div className="flex flex-col h-full">
                       <IngredientListPanel isCollapsed={false} />
                    </div>
                </div>
                {isMobileMenuOpen && <div className="fixed md:hidden inset-0 bg-black/50 z-20" onClick={() => setIsMobileMenuOpen(false)}></div>}
